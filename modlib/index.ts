...
export function IndexOfFirstTrue(
  array: mod.Array,
...
export function ShowEventGameModeMessage(
  event: mod.Message,
  target?: mod.Player | mod.Team
) {
  ...
  const MakeShiftDisplayGameModeMessage = async (
    message: mod.Message,
    target?: mod.Player | mod.Team
  ) => {
...
export function ShowHighlightedGameModeMessage(
  event: mod.Message,
  target?: mod.Player | mod.Team
) {
  if (target) {
...
export function ClearCustomNotificationMessage(
  ...
  try {
    if (target) {
      ...
      if (mod.IsType(target, mod.Types.Player)) {
        mod.DeleteUIWidget(
          mod.FindUIWidgetWithName(custom + String(target as mod.Player))
        );
      }
